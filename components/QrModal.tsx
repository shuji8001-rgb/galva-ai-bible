...
export const QrModal: React.FC<QrModalProps> = ({
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-md bg-[#0b1120] border-2 border-cyan-500/50 rounded-2xl p-6 shadow-2xl shadow-cyan-500/20 text-white"
      ...
      >
        ...
        <div className="flex items-center gap-2.5 mb-4 border-b border-slate-800 pb-3">
          ...
          <div>
            ...
            <p className="text-xs text-cyan-300/80 font-medium line-clamp-1">{title}</p>
          </div>
        </div>

        {/* QRコード画像コンテナ */}
        <div className="flex flex-col items-center justify-center p-5 bg-gradient-to-b from-slate-900 to-[#070d18] rounded-xl border border-slate-800/80 shadow-inner mb-4">
          <div className="p-3 bg-white rounded-xl shadow-lg ring-2 ring-cyan-500/40">
          ...
          <div className="flex items-center gap-1.5 mt-3.5 text-xs text-slate-300 font-medium text-center">
            ...
            <span>スマホのカメラで読み取って、現場・工場で即座に活用</span>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2 bg-slate-950/80 border border-slate-700/80 rounded-xl p-1.5 pl-3">
            <span className="text-xs font-mono text-cyan-300 truncate flex-1 select-all">
              {currentUrl}
            </span>
            <button
              onClick={handleCopy}
