...
-- 3. インデックス作成（検索高速化）
CREATE INDEX IF NOT EXISTS idx_galva_questions_no ON public.questions_queue(no);
CREATE INDEX IF NOT EXISTS idx_galva_questions_created ON public.questions_queue(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_galva_knowledge_qid ON public.knowledge_records(question_id);
CREATE INDEX IF NOT EXISTS idx_galva_knowledge_created ON public.knowledge_records(created_at DESC);
...
VALUES ('galva-media', 'galva-media', true)
ON CONFLICT (id) DO NOTHING;

CREATE POLICY "Allow public read galva-media" ON storage.objects FOR SELECT USING (bucket_id = 'galva-media');
CREATE POLICY "Allow public insert galva-media" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'galva-media');
CREATE POLICY "Allow public update galva-media" ON storage.objects FOR UPDATE USING (bucket_id = 'galva-media');
CREATE POLICY "Allow public delete galva-media" ON storage.objects FOR DELETE USING (bucket_id = 'galva-media');
